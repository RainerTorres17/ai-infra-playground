...
Each training job registers a new model version in MLflow. The first run promotes a model to the `@prod` alias automatically for serving. The consecutive runs are promoted to prod by the training script under the condition that their SMAPE score is lower than the current production model. 
...
-	Replace ARIMA with a regression or Prophet-based model
